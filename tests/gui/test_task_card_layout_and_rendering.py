"""작업 카드 4분면 레이아웃, 썸네일 렌더링, 상태별 시각 하이라이트 및 목록 추가 GUI 테스트."""
...
@pytest.mark.ticket("T0104")
def test_vod_info_display_name_with_live_date() -> None:
    """[T0104] 라이브 시작일(live_open_date)이 존재할 때 '[스트리머] YYYY-MM-DD 제목' 규칙 검증."""
    info = VodInfo(
        video_no="15016450",
...
@pytest.mark.ticket("T0104")
def test_vod_info_display_name_without_live_date() -> None:
    """[T0104] 라이브 시작일이 없을 때 '[스트리머] 제목' 규칙 검증."""
    info = VodInfo(
        video_no="99999",
...
@pytest.mark.ticket("T0104")
def test_format_duration_helper() -> None:
    """[T0104] 초 단위 재생 시간 포맷 변환 검증."""
...
@pytest.mark.ticket("T0104")
def test_task_card_analyzing_state_and_hover_action(qtbot) -> None:
    """[T0104] 분석 진행 상태(ANALYZING)의 카드 레이아웃, 줄바꿈, 호버 시 액션 아이콘 표시 검증."""
...
@pytest.mark.ticket("T0104")
def test_task_card_ready_state_and_update(qtbot) -> None:
    """[T0104] 분석 완료(READY) 상태 갱신 시 메타데이터 및 화질/시간 표시 검증."""
    url = "https://chzzk.naver.com/video/15016450"
    card = TaskCardWidget(raw_url=url, status=TaskStatus.ANALYZING)
    qtbot.addWidget(card)
...
@pytest.mark.ticket("T0104")
def test_task_card_thumbnail_loading(qtbot) -> None:
    """[T0104] 썸네일 바이트 로드 시 QLabel에 QPixmap이 반영되는지 검증."""
    card = TaskCardWidget(
...
@pytest.mark.ticket("T0104")
def test_task_card_failed_invalid_styling(qtbot) -> None:
    """[T0104] 유효하지 않은 URL 또는 분석 실패(FAILED_INVALID) 시 빨간색 하이라이트 검증."""
...
@pytest.mark.ticket("T0104")
def test_task_card_failed_login_required_styling(qtbot) -> None:
    """[T0104] 로그인 필요(FAILED_LOGIN_REQUIRED) 시 빨간색 하이라이트 및 인증 영역 노출 검증."""
...
@pytest.mark.ticket("T0104")
def test_task_card_delete_requested_signal(qtbot) -> None:
    """[T0104] 카드 내 삭제(✕) 버튼 클릭 시 delete_requested 시그널 방출 검증."""
...
@pytest.mark.ticket("T0104")
def test_task_list_add_at_top_and_delete(qtbot) -> None:
    """[T0104] 새 작업 카드가 목록 최상단(Index 0)에 추가되어 오래된 항목이 아래로 가는지 검증."""
...
def test_main_window_invalid_url_immediately_adds_failed_card_and_toast(
    ...
    """[T0104] 잘못된 URL 입력 시 작업 목록 최상단에 빨간색 카드가 즉시 추가되고 토스트가 뜨는지 검증."""
...
@pytest.mark.ticket("T0104")
def test_main_window_valid_url_success_flow(main_window, qtbot) -> None:
    """[T0104] 정상 치지직 VOD URL 입력 시 분석 중 카드 등록 후 라이브 시작일 포함 메타데이터로 갱신 검증."""
    mock_vod = VodInfo(
        video_no="15016450",
...
@pytest.mark.ticket("T0104")
def test_main_window_valid_url_login_required_flow(main_window, qtbot) -> None:
    """[T0104] 성인인증/로그인 필요 오류 발생 시 카드가 빨간색으로 하이라이트되고 안내 문구가 반영되는지 검증."""
...
@pytest.mark.ticket("T0104")
def test_main_window_401_unauthorized_triggers_login_required(
    main_window, qtbot
) -> None:
    """[T0104] 치지직 쿠키 필요 VOD의 401 Unauthorized 오류 시 Login required 상태로 전환되는지 검증."""
...
@pytest.mark.ticket("T0104")
def test_main_window_valid_url_not_found_flow(main_window, qtbot) -> None:
    """[T0104] VOD 404 미존재 오류 발생 시 카드가 빨간색 실패로 갱신되고 Invalid 토스트가 뜨는지 검증."""
